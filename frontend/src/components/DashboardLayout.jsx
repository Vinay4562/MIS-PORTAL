  ...
  const SidebarContent = ({ collapsed }) => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        <button
          onClick={() => {
            navigate('/station-loads');
            setOpen(false);
          }}
          className={`w-full flex items-center ${collapsed ? 'justify-center' : 'gap-3'} px-4 py-3 rounded-lg transition-colors text-left ${
            location.pathname === '/station-loads'
              ? 'bg-blue-600 text-white'
              : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'
          }`}
          data-testid="station-loads-menu"
          title={collapsed ? "STATION LOADS" : undefined}
        >
          <FileText className="w-5 h-5" />
          {!collapsed && <span className="font-medium">STATION LOADS</span>}
        </button>
