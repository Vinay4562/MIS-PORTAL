      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
            <thead className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700">
                <tr>
                    <th className="px-4 py-3 font-medium border-r border-slate-200 dark:border-slate-700 sticky left-0 bg-slate-100 dark:bg-slate-800 z-10">Date</th>
                    ...
                    <th className="px-4 py-3 font-medium text-right border-r">Total</th>
                    <th className="px-4 py-3 font-medium text-center sticky right-0 bg-slate-100 dark:bg-slate-800 z-10">Actions</th>
                </tr>
                <tr className="text-xs text-slate-500 dark:text-slate-400">
                    <th className="sticky left-0 bg-slate-100 dark:bg-slate-800 border-r z-10"></th>
                    {sheet.meters.map(meter => (
                        <>
                            <th className="px-2 py-2 text-right font-normal border-r min-w-[80px]">Initial</th>
                            <th className="px-2 py-2 text-right font-normal border-r min-w-[80px]">Final</th>
                            <th className="px-2 py-2 text-right font-normal border-r min-w-[50px]">MF</th>
                            <th className="px-2 py-2 text-right font-normal border-r min-w-[80px]">Cons.</th>
                        </>
                    ))}
                    <th className="px-4 py-2 text-right font-normal border-r">Consumption</th>
                    <th className="sticky right-0 bg-slate-100 dark:bg-slate-800 z-10"></th>
                </tr>
            </thead>
                ...
                {entries.map(entry => (
                    <tr key={entry.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 group">
                        ...
                        {sheet.meters.map(meter => {
                            ...
                            return (
                                <>
                                    <td className="px-2 py-3 text-right border-r text-slate-500">{reading?.initial?.toFixed(2) || '-'}</td>
                                    <td className="px-2 py-3 text-right border-r font-medium">{reading?.final?.toFixed(2) || '-'}</td>
                                    <td className="px-2 py-3 text-right border-r text-slate-400">{meter.mf}</td>
                                    <td className="px-2 py-3 text-right border-r font-bold text-blue-600 dark:text-blue-400">
                                        {reading?.consumption?.toFixed(2) || '-'}
                                    </td>
                                </>
                            );
                        })}
                        <td className="px-4 py-3 text-right font-bold text-slate-900 dark:text-white border-r">
